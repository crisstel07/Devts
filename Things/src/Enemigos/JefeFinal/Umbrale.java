...
public class Umbrale extends EnemigoBase {
...
private boolean estaMuriendo = false;
    ...
    public Umbrale(int x, int y, Jugador jugador) {
    ...
    private void cargarAnimaciones() {
        try {
            animIdle = new Animacion(cargarSprites("idle", 4), 20, true);
            animGenerar = new Animacion(cargarSprites("Generacion", 8), 10, false);
            animCaminar = new Animacion(cargarSprites("invocacion", 8), 24, true);
            animacionMuerte = new Animacion(cargarSprites("MuerteFase1", 6), 10, false);  // Usa tus propios nombres y cantidad de frames
    ...
    private BufferedImage[] cargarSprites(String baseNombre, int cantidad) throws IOException {
        ...
        for (int i = 0; i < cantidad; i++) {
            frames[i] = ImageIO.read(Objects.requireNonNull(getClass().getResource("/Graficos/Sprites/Enemigos/Umbrale/" + baseNombre + "_" + i + ".png")));
        }
        return frames;
    }
